...
import { SAMPLE_QUESTIONS, SPECIES_DB } from '../constants';
...
const IdentificationGames: React.FC<Props> = ({ mode, selectedGroups, onExit }) => {
  ...
  // Laskee nimen perusteella numeron, jotta samalle lajille tulee aina sama kuva
  ...
  useEffect(() => {
    // Suodatetaan tietokanta valittujen ryhmien perusteella
    const filteredSpecies = SPECIES_DB.filter(s => selectedGroups.includes(s.group));
    
    // Generoidaan kysymykset vain valituista lajeista
    const generatedQuestions: Question[] = filteredSpecies.map((s, idx) => {
    ...
    setSpeedAnswers(new Array(generatedQuestions.length).fill(null));
  }, [selectedGroups]);

  const handleFlip = () => {
    if (mode === 'flashcard' && !isTransitioning) {
            ...
            >
              ...
              <div className="absolute inset-0 backface-hidden bg-emerald-900 rounded-3xl shadow-2xl p-10 flex flex-col items-center justify-center text-center rotate-y-180 border-4 border-emerald-800">
                ...
                <button 
                  ...
                  className={`px-12 py-4 bg-white text-emerald-900 rounded-full font-black uppercase tracking-widest transition-all transform active:scale-95 shadow-xl ${isTransitioning ? 'opacity-50' : 'hover:bg-emerald-50'}`}
                >
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-stone-200">
               <div className="h-64 md:h-80 relative">
                 ...
                 {speedAnswers[currentIndex] !== null && (
                   <div className={`absolute inset-0 flex items-center justify-center backdrop-blur-sm transition-all duration-300 ${speedAnswers[currentIndex] === current.correctIndex ? 'bg-emerald-500/30' : 'bg-rose-500/30'}`}>
                      <div className={`p-8 rounded-full shadow-2xl scale-110 ${speedAnswers[currentIndex] === current.correctIndex ? 'bg-emerald-600' : 'bg-rose-600'}`}>
